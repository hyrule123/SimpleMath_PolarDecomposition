...
//https://eprints.maths.manchester.ac.uk/694/1/high86p.pdf
	...
	Matrix Matrix::Normalize(const Matrix& _m) const
	{
		Matrix ret = _m;
		...
		for (int i = 0; i < 4; ++i)
		{
			...
			{
				//제곱수를 전부 더해준다
				length += _m.m[i][j] * _m.m[i][j];
			}
		}

		const float factor = 1.f / sqrtf(length) + std::numeric_limits<float>::min();
 ...
 * @return the norm
 */
	float Matrix::NormDividedBy1() const
	{
		float norm = 0.f;
	...
	Matrix Matrix::Interpolate(const Matrix& _m, float _ratio)
	{
		return Interpolate(*this, _m, _ratio);;
	}

	Matrix Matrix::Interpolate(const Matrix& _m1, const Matrix& _m2, float _ratio)
	{
		Matrix ret{};
		for (int i = 0; i < 4; ++i)
		{
			for (int j = 0; j < 4; ++j) 
			{
				ret.m[i][j] = (_m2.m[i][j] - _m1.m[i][j]) * _ratio + _m1.m[i][j];
			}
		}
		return ret;
	}
